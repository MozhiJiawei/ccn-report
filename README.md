...
每个报告目录的 `README.md` 必须包含一句独立、完整、可直接引用的技术总结，回答“这个技术是什么”。总结需明确技术对象与目标场景，包含来源可验证的指标、范围或验证状态，只陈述当前版本已实现或论文已验证的能力，说明直接价值，并注明版本、发布日期或其他有效时间边界。
...
脚本会读取滚动 Release 的 manifest，先用其中的 SHA256 校验本地月度 ZIP 缓存，只下载缺失或内容已变化的分包。所有分包会先完整解压到临时目录，再归并到输出目录：同路径文件直接覆盖，新文件加入，仅在本地存在的旧文件保留。默认缓存位于输出目录旁的 `.ccn-report-cache`，可通过 `--cache-dir` 指定其他位置，也可通过 `--github-repo`、`--tag` 和 `--output-dir` 指定其他仓库、发布标签和输出位置。
