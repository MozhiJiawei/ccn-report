# Visual Review Checklist

## use-case-catalog-view

- Export: `.tmp\generate-3plus1-diagrams\loop-engineering-analysis\use-case\exports\use-case-catalog-view.png`
- Reference: not found
- Metrics: `1963x977`, non-white pixel ratio `0.7523`

```text
AI visual review task:
1. You must load the exported preview image: .tmp\generate-3plus1-diagrams\loop-engineering-analysis\use-case\exports\use-case-catalog-view.png
2. No matching reference image was found; compare against references/style-profiles.md and references/ref-usage.md instead.
3. Current preview metrics: size 1963x977, non-white pixel ratio 0.7523.
4. State explicitly that you reviewed both the export and the reference before judging quality.
5. Compare structure layering, grouping semantics, visual density, label language and readability, edge routing, color discipline, and overall visual order.
6. Call out concrete issues such as large blank areas, unbalanced layout, weak grouping semantics, oversized labels, inconsistent colors, missing relationships, or unreadable labels.
7. Prefer suggesting changes to intermediate-model fields or renderer layout rules; if suggesting edge removal, keep it minimal rather than deleting all cross-group edges.
8. After changes, export again and review again; do not stop at one round of commentary.
```

## use-case-view

- Export: `.tmp\generate-3plus1-diagrams\loop-engineering-analysis\use-case\exports\use-case-view.png`
- Reference: not found
- Metrics: `1633x1033`, non-white pixel ratio `0.1672`

```text
AI visual review task:
1. You must load the exported preview image: .tmp\generate-3plus1-diagrams\loop-engineering-analysis\use-case\exports\use-case-view.png
2. No matching reference image was found; compare against references/style-profiles.md and references/ref-usage.md instead.
3. Current preview metrics: size 1633x1033, non-white pixel ratio 0.1672.
4. State explicitly that you reviewed both the export and the reference before judging quality.
5. Compare structure layering, grouping semantics, visual density, label language and readability, edge routing, color discipline, and overall visual order.
6. Call out concrete issues such as large blank areas, unbalanced layout, weak grouping semantics, oversized labels, inconsistent colors, missing relationships, or unreadable labels.
7. Prefer suggesting changes to intermediate-model fields or renderer layout rules; if suggesting edge removal, keep it minimal rather than deleting all cross-group edges.
8. After changes, export again and review again; do not stop at one round of commentary.
```
