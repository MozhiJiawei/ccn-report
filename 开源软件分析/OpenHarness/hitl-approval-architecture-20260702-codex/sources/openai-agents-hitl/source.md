...
published_date: Not specified on page
captured_date: 2026-07-02
capture_mode: rendered capture
---

# Source Metadata

- Source URL: https://openai.github.io/openai-agents-python/human_in_the_loop/
- Title: OpenAI Agents SDK Human-in-the-loop
- Page title: Human-in-the-loop - OpenAI Agents SDK
- Publisher: OpenAI
- Author: Not specified on page
- Published date: Not specified on page
- Captured date: 2026-07-02
- Capture mode: rendered capture

# Capture Notes

- Body boundary: extracted the rendered `article` element containing the `Human-in-the-loop` page content; excluded header navigation, left documentation navigation, right table of contents, and footer/adjacent page chrome.
- Tail audit: final captured section is `Versioning pending tasks`, which remains part of the article body.
- Media summary: no images were present inside the article body after full-page scroll/lazy-load pass. The site logo and navigation UI imagery were rejected as non-body chrome, so `images/` is intentionally empty.
...
`needs_approval` is available on [`function_tool`](https://openai.github.io/openai-agents-python/ref/tool/#agents.tool.function_tool), [`Agent.as_tool`](https://openai.github.io/openai-agents-python/ref/agent/#agents.agent.Agent.as_tool), [`ShellTool`](https://openai.github.io/openai-agents-python/ref/tool/#agents.tool.ShellTool), and [`ApplyPatchTool`](https://openai.github.io/openai-agents-python/ref/tool/#agents.tool.ApplyPatchTool). Local MCP servers also support approvals through `require_approval` on [`MCPServerStdio`](https://openai.github.io/openai-agents-python/ref/mcp/server/#agents.mcp.server.MCPServerStdio), [`MCPServerSse`](https://openai.github.io/openai-agents-python/ref/mcp/server/#agents.mcp.server.MCPServerSse), and [`MCPServerStreamableHttp`](https://openai.github.io/openai-agents-python/ref/mcp/server/#agents.mcp.server.MCPServerStreamableHttp). Hosted MCP servers support approvals via [`HostedMCPTool`](https://openai.github.io/openai-agents-python/ref/tool/#agents.tool.HostedMCPTool) with `tool_config={"require_approval": "always"}` and an optional `on_approval_request` callback. Shell and apply_patch tools accept an `on_approval` callback if you want to auto-approve or auto-reject without surfacing an interruption.
...
2. If an approval decision for that tool call is already stored in the `RunContextWrapper` , the runner proceeds without prompting. Per-call approvals are scoped to the specific call ID; pass always_approve=True or always_reject=True to persist the same decision for future calls to that tool during the rest of the run.
3. Otherwise, execution pauses and RunResult.interruptions (or RunResultStreaming.interruptions ) contains `ToolApprovalItem` entries with details such as agent.name , tool_name , and arguments . This includes approvals raised after a handoff or inside nested Agent.as_tool() executions.
...
- Run-wide fallback: set `RunConfig.tool_error_formatter` to control the default model-visible message for approval rejections across the whole run.
...
- Local `ShellTool` and `ApplyPatchTool` can use on_approval to approve or reject immediately in code.
- `HostedMCPTool` can use tool_config={"require_approval": "always"} together with on_approval_request for the same kind of programmatic decision.
- Plain `function_tool` tools and `Agent.as_tool()` use the manual interruption flow on this page.
...
The same interruption flow works in streaming runs. After a streamed run pauses, keep consuming [`RunResultStreaming.stream_events()`](https://openai.github.io/openai-agents-python/ref/result/#agents.result.RunResultStreaming.stream_events) until the iterator finishes, inspect [`RunResultStreaming.interruptions`](https://openai.github.io/openai-agents-python/ref/result/#agents.result.RunResultStreaming.interruptions), resolve them, and resume with [`Runner.run_streamed(...)`](https://openai.github.io/openai-agents-python/ref/run/#agents.run.Runner.run_streamed) if you want the resumed output to keep streaming. See [Streaming](https://openai.github.io/openai-agents-python/streaming/) for the streamed version of this pattern.
If you are also using a session, keep passing the same session instance when you resume from `RunState`, or pass another session object that points at the same backing store. The resumed turn is then appended to the same stored conversation history. See [Sessions](https://openai.github.io/openai-agents-python/sessions/) for the session lifecycle details.
...
- Local shell and apply_patch tools : ShellTool and ApplyPatchTool also support needs_approval . Use state.approve(interruption, always_approve=True) or state.reject(..., always_reject=True) to cache the decision for future calls. For automatic decisions, provide on_approval (see examples/tools/shell.py ); for manual decisions, handle interruptions (see examples/tools/shell_human_in_the_loop.py ). Hosted shell environments do not support needs_approval or on_approval ; see the tools guide .
...
- Realtime agents : The realtime demo exposes WebSocket messages that approve or reject tool calls via approve_tool_call / reject_tool_call on the RealtimeSession (see examples/realtime/app/server.py for the server-side handlers and Realtime guide for the API surface).
