...
> 
> When invoking the agent, pass a `config` that includes the **thread ID** to associate execution with a conversation thread. See the [LangGraph interrupts documentation](https://docs.langchain.com/oss/python/langgraph/interrupts) for details.
...
- Publisher/site: Docs by LangChain.
- Capture mode: rendered capture via the Codex in-app Browser from the live LangChain docs page.
- Body boundary: captured the documentation article body under `#content`, including the page title/header context and stopping before site helper modules such as “Connect these docs”, “Edit this page”, issue links, navigation, footer, and sidebars.
- Media summary: no正文/body images were present in the rendered article container after scrolling for lazy-loaded assets. The `images/` directory is intentionally empty.
- Reliability note: code blocks, tables, callouts, details/accordion content, and tabbed decision examples were extracted from the rendered DOM. Hidden tab panels were included when present in the article DOM because they are part of the docs reading experience.
